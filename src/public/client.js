...
const renderHTML = (rovers, roverNames, selectedRover, photos) => {
  const header = select(roverNames, selectedRover);
  const content = roverData(rovers, selectedRover, photos);
  ...
  if (roverPhotos) {
    return `
            <section class="rover-photo">
                <p> *The following photos were taken on ${formatDate(
                  max_date
                )}.</p>
                <div class="photos">
                    ${roverPhotos
